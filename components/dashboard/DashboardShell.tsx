...
import {
  Bot, Plus, Upload, Bell, Zap, ChevronUp, Menu, X, LayoutDashboard, Timer
} from "lucide-react";
    ...
    <div className="flex h-screen overflow-hidden bg-gray-50 text-gray-900">
      ...
      >
        <div className="flex items-center gap-3 px-5 py-4 border-b border-white/5">
          <div className="flex h-9 w-9 items-center justify-center shrink-0 rounded-lg overflow-hidden bg-white/5">
            <img src="/assets/images/header/og-image.png" alt="AABW 2026 Logo" className="h-full w-full object-contain" />
          </div>
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
          {[{ icon: LayoutDashboard, label: "Dashboard", active: true }].map(({ icon: Icon, label, active }) => (
            <button
              key={label}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-20 flex h-14 items-center gap-3 px-4 border-b border-gray-200 bg-white/95 backdrop-blur-sm">
          <button className="lg:hidden text-gray-500 hover:text-gray-900 mr-1" onClick={() => setSidebarOpen(true)}>
          ...
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" onClick={() => setParseModalOpen(true)} className="hidden sm:flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-900 h-8">
              <Upload className="h-3.5 w-3.5" /> Parse
            </Button>

            <Button size="sm" onClick={() => { setEditingDeadline(null); setDeadlineModalOpen(true); }} className="h-8 gap-1.5 text-xs bg-green-600 hover:bg-green-700 text-white border-0">
              <Plus className="h-3.5 w-3.5" /> <span className="hidden sm:inline">Add Deadline</span> <span className="sm:hidden">Add</span>
            </Button>
        ...
        >
          <div className="w-full bg-black border-b border-gray-800">
            <img src="/assets/images/header/full_width.png" alt="AABW Sponsors and Partners" className="w-full h-auto max-h-[140px] object-cover sm:object-contain object-center" />
          </div>
          ...
          <div className="px-6 pb-8">
            <div className="flex items-center justify-between mb-4">
              ...
              <Button variant="ghost" size="sm" onClick={() => setParseModalOpen(true)} className="sm:hidden h-8 gap-1.5 text-xs text-gray-600">
                <Upload className="h-3.5 w-3.5" /> Parse
              </Button>
            </div>
      ...
      <div className={`fixed bottom-6 z-30 flex flex-col gap-3 transition-all duration-300 ${chatOpen ? "right-6 lg:right-[408px]" : "right-6"}`}>
        {showScrollTop && (
          <Button size="icon" onClick={scrollToTop} className="h-12 w-12 rounded-full shadow-xl bg-white border border-gray-200 text-gray-600 hover:text-gray-900 hover:bg-gray-50 transition-all" title="Back to top">
            <ChevronUp className="h-5 w-5" />
          </Button>
        )}

        <Button size="icon" onClick={() => setChatOpen((v) => !v)} className={`h-12 w-12 rounded-full shadow-xl transition-all ${chatOpen ? "bg-green-600 hover:bg-green-700 text-white border-0" : "bg-white border border-gray-200 text-green-600 hover:bg-green-50"}`} title="AI Assistant">
          <Bot className="h-5 w-5" />
        </Button>
      </div>

      {/* ── MODALS ── */}
      <DeadlineModal open={deadlineModalOpen} deadline={editingDeadline} onClose={handleCloseDeadlineModal} onSaved={mutate} />
      <ParseUploadModal open={parseModalOpen} onClose={() => setParseModalOpen(false)} onSaved={mutate} />
    </div>
  );
}