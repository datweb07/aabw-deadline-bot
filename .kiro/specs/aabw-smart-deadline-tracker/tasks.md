...
- [ ] 1. Project scaffolding and configuration
  ...
  - [ ] 1.2 Install and configure all dependencies (Lowdb, Vercel AI SDK, Zod, SWR, Lucide React, uuid)
  - [ ] 1.3 Create `.env.example` and `next.config.ts` with required environment variable documentation
  - [ ] 1.4 Set up `tsconfig.json` path aliases and Tailwind config with custom color tokens

- [ ] 2. Core data layer
  Depends on: 1
  - [ ] 2.1 Define all TypeScript interfaces in `lib/types.ts` (Deadline, AppNotification, DatabaseSchema, DeadlineCategory)
  - [ ] 2.2 Create `data/mockData.json` with 30+ pre-seeded AABW 2026 deadlines across all 5 days
  - [ ] 2.3 Create `lib/db.ts` Lowdb singleton with seed-on-empty initialization from mockData.json
  - [ ] 2.4 Create `lib/utils.ts` with date formatting, time-remaining, and category color helpers

- [ ] 3. Backend API routes
  Depends on: 2
  - [ ] 3.1 Implement `GET /api/deadlines` and `POST /api/deadlines` in `app/api/deadlines/route.ts`
  - [ ] 3.2 Implement `PUT /api/deadlines/[id]` and `DELETE /api/deadlines/[id]` in `app/api/deadlines/[id]/route.ts`
  - [ ] 3.3 Implement `POST /api/parse` AI schedule smart-parser with Vercel AI SDK `generateObject` and Zod schema
  - [ ] 3.4 Implement `POST /api/chat` streaming chat agent with Vercel AI SDK `streamText` and four tool definitions
